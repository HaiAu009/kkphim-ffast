...
use KKPhim\ThemeFfast\Controllers\ThemeFfastController;
...
Route::group([
    ...
    Route::post(sprintf('/%s/{movie}/{episode}-{id}/report', config('kkphim.routes.movie', 'ajax')), [ThemeFfastController::class, 'reportEpisode'])
        ->where(['movie' => '.+', 'episode' => '.+', 'id' => '[0-9]+'])->name('episodes.report');
    Route::post(sprintf('/%s/{movie}/rate', config('kkphim.routes.movie', 'ajax')), [ThemeFfastController::class, 'rateMovie'])->name('movie.rating');
